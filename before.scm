...
(define less-of-two (lambda (a b) (not (or (math-eq-two a b) (math-greater-two a b)))))
(define < (lambda lq (letrec ((rec (lambda (lst) (if (null? (cdr lst)) #t (and (less-of-two (car lst) (car (cdr lst))) (rec (cdr lst)))))))   (rec lq))))
...
(define myreverseforapply (lambda (lst) (if (null? lst) '() (append (myreverseforapply (cdr lst)) (list (car lst))))))
(define apply (lambda (f lst) (apply-helper-runtime f (myreverseforapply lst))))
