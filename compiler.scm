...
(define type-const
  (lambda (e)
    ...
    ;(display "\n")
    (cond
      ((or (char? e) (number? e) (string? e) (null? e) (boolean? e) (equal? e `,void-object)) `(,e)); was void?
      ((pair? e)
...
;                (display ":;\n")
;                (display ":type-const-vector:")
;                (display (type-const vector-of-consts))
;                (display ":;\n")
...
(define make-eq?
  (lambda ()
    (let ((body-lab "eq_BODY")
      ...
      (let ((char_object (lab-construct "CHAR_OBJECT"))
            (symbol_object (lab-construct "SYMBOL_OBJECT"))
            (integer_object (lab-construct "INTEGER_OBJECT"))         
            (check_R4_integer (lab-construct "CHECK_R4_INTEGER"))  
            (frac_object (lab-construct "FRAC_OBJECT"))
            (check_R4_frac (lab-construct "CHECK_R4_FRAC"))
            (cmp_strings (lab-construct "CMP_STRINGS"))
            (void_object (lab-construct "VOID_OBJECT"))
            (nil_object (lab-construct "NIL_OBJECT"))
            (bool_object (lab-construct "BOOL_OBJECT"))
            (string_object (lab-construct "STRING_OBJECT"))
            (vector_object (lab-construct "VECTOR_OBJECT"))
            (pair_object (lab-construct "PAIR_OBJECT"))
            (not_equal (lab-construct "NOT_EQUAL"))
            (equal (lab-construct "EQUAL"))
            (exit (lab-construct "EXIT")))
        
        (ltc (mov "R3" (fparg "2")))
        (ltc (mov "R4" (fparg "3")))        
        (ltc (cmp (ind "R3") (imm "T_SYMBOL")))
        (ltc (jmp-eq symbol_object))
        (ltc (cmp (ind "R3") (imm "T_CHAR")))
        (ltc (jmp-eq char_object))
        (ltc (cmp (ind "R3") (imm "T_INTEGER")))
        (ltc (jmp-eq integer_object))
        (ltc (cmp (ind "R3") (imm "T_FRACTION")))
        (ltc (jmp-eq frac_object))
;        (ltc (cmp (ind "R3") (imm "T_VOID")))
;        (ltc (jmp-eq void_object))
;        (ltc (cmp (ind "R3") (imm "T_NIL")))
;        (ltc (jmp-eq nil_object))
;        (ltc (cmp (ind "R3") (imm "T_BOOL")))
;        (ltc (jmp-eq bool_object))
;        (ltc (cmp (ind "R3") (imm "T_STRING")))
;        (ltc (jmp-eq string_object))
;        (ltc (cmp (ind "R3") (imm "T_VECTOR")))
;        (ltc (jmp-eq vector_object))
;        (ltc (cmp (ind "R3") (imm "T_PAIR")))
;        (ltc (jmp-eq pair_object))
        (ltc (cmp  "R3" "R4"))
        (ltc (jmp-ne not_equal))
        (ltc (jmp equal))
        
         (labtc symbol_object)
        (ltc (cmp (ind "R4") (imm "T_SYMBOL")))
        (ltc (jmp-ne not_equal))
        (ltc (cmp (indd "R3" (imm "1")) (indd "R4" (imm "1"))))
        (ltc (jmp-ne cmp_strings))
        (ltc (jmp equal))

        (labtc cmp_strings)
        (ltc (push (indd "R3" (imm "1"))))
        (ltc (push (indd "R4" (imm "1"))))
        (ltc (call "IS_STRINGS_EQUAL"))
        (ltc (drop "2"))        
        (ltc (cmp "R0" (imm "1")))
        (ltc (jmp-ne not_equal))
        (ltc (jmp equal))
        ...
        (labtc char_object)
        (ltc (cmp (ind "R4") (imm "T_CHAR")))
        (ltc (jmp-ne not_equal))
        (ltc (cmp (indd "R3" (imm "1")) (indd "R4" (imm "1"))))
        (ltc (jmp-ne not_equal))
        (ltc (jmp equal))

        (labtc integer_object)
        (ltc (cmp (ind "R4") (imm "T_INTEGER")))
        (ltc (jmp-ne check_R4_frac))
        (ltc (cmp (indd "R3" (imm "1")) (indd "R4" (imm "1"))))
        (ltc (jmp-ne not_equal))
        (ltc (jmp equal))

        (labtc frac_object)
        (ltc (cmp (ind "R4") (imm "T_FRACTION")))
        (ltc (jmp-ne check_R4_integer))
        (ltc (cmp (indd "R3" (imm "1")) (indd "R4" (imm "1"))))
        (ltc (jmp-ne not_equal))
        (ltc (cmp (indd "R3" (imm "2")) (indd "R4" (imm "2"))))
        (ltc (jmp-ne not_equal))
        (ltc (jmp equal))
        ...
        (ltc (mov "R3" "R4"))
        (ltc (mov "R4" "R5"))
        (ltc (jmp check_R4_frac))
        
        
        (labtc check_R4_frac)
        (let ((nomi_0_R4 (lab-construct "nomi_0_R4"))
              (denomi_1_R4 (lab-construct "denomi_1_R4"))
              (exit (lab-construct "EXIT")))
          (ltc (cmp (ind "R4") (imm "T_FRACTION")))
          (ltc (jmp-ne not_equal))
          (ltc (cmp (indd "R4" (imm "1")) (imm "0")))        
          (ltc (jmp-eq nomi_0_R4))
          (ltc (cmp (indd "R4" (imm "2")) (imm "1")))
          (ltc (jmp-eq denomi_1_R4))

          (labtc nomi_0_R4)
          (ltc (cmp (indd "R3" (imm "1")) (imm "0")))
          (ltc (jmp-ne not_equal))
          (ltc (jmp equal))
  ...
  (lambda ()
    (let ((body-lab "LStringToSymbolBody")
      ...
      (let ((found (lab-construct "FOUND"))
            ...
            (not_equal_strings (lab-construct "NOT_EQUAL_STRINGS"))
            (exit (lab-construct "EXIT")))
        (ltc (push "R1"))
        (ltc (push "R2"))
        (ltc (push "R3"))
...
(define make-make-string-runtime-help
  (lambda (proc-name type body-lab closure-lab)
    ...
    (let ((morethanone (lab-construct "MORE_THAN_ONE_ARG_MAKE_STR"))
          (gotoloop (lab-construct "GOTOLOOPSTR")))
      (ltc (cmp (fparg "1") "1"))
      (ltc (jmp-ne morethanone))
      (ltc (mov "R2" "0"))
      (ltc (jmp gotoloop))
      (labtc morethanone)
      (ltc (mov "R2" (indd (fparg "3") "1")))
...
(define make-make-vector-runtime-help
  (lambda (proc-name type body-lab closure-lab)
    ...
    (ltc (mov "FP" "SP"))     
    (ltc (mov "R1" (indd (fparg "2") "1")))
    (let ((morethanone (lab-construct "MORE_THAN_ONE_ARG_MAKE_VEC"))
          (gotoloop (lab-construct "GOTOLOOP")))
      (ltc (cmp (fparg "1") "1"))
      (ltc (jmp-ne morethanone))
      (ltc (push "0"))
      (ltc (call "MAKE_SOB_INTEGER"))
